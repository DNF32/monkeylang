...
spec = do
  describe "Position tracking" $ do
    describe "Single character parsing" $ do
      ...
      it "handles multiple newlines" $ do
        let result = runLexer (oneOrMore (anyNonWhitespaceLexer <|> nl)) (initialState "\n\n\n")
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 4 1
          Left _           -> expectationFailure "Lexer failed"

      it "tracks position across lines" $ do
        let result = runLexer (oneOrMore (anyNonWhitespaceLexer <|> nl)) (initialState "ab\ncddd")
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 2 5
          Left _           -> expectationFailure "Lexer failed"
  ...
  describe "Error position tracking" $ do
    it "reports correct position on error" $ do
      ...
      case result of
        Left err -> errorPosition err `shouldBe` Position 1 1
    ...
    it "reports correct position on error mid-input" $ do
      ...
      case result of
        Left err -> errorPosition err `shouldBe` Position 1 3
        Right _  -> expectationFailure "Should have failed"
    ...
    it "reports correct position on error after newline" $ do
      ...
      case result of
        Left err -> errorPosition err `shouldBe` Position 2 1
        Right _  -> expectationFailure "Should have failed"
