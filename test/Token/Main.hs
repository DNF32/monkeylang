...
spec = do
  describe "Position tracking" $ do
    describe "Single character parsing" $ do
      it "starts at line 1, column 1" $ do
        ...
        currentPosition state `shouldBe` Position 1 1 0

      it "advances column by 1 when cosuming 1 char" $ do
        let lexerA = charL 'a'
        let result = runLexer lexerA (initialState "a")
        case result of
          Right (_, newState) -> currentPosition newState `shouldBe` Position 1 2 1
      ...
      it "advances column by 1 for multiple chars" $ do
        ...
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 1 4 3
          Left _ -> expectationFailure "Lexer failed"
    ...
    describe "Tab handling" $ do
      it "advances column by 4 for tab" $ do
        ...
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 1 5 4
          Left _ -> expectationFailure "Lexer failed"
      ...
      it "handles tabs and spaces mixed" $ do
        ...
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 1 10 9 -- 4 + 1 + 4 + 1
          Left _ -> expectationFailure "Lexer failed"
    ...
    describe "Carriage return handling" $ do
      it "ignores carriage return (position unchanged)" $ do
        ...
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 1 1 0
          Left _ -> expectationFailure "Lexer failed"
      ...
      it "handles CRLF correctly" $ do
        ...
        let result = runLexer lexer (initialState "\r\n")
        case result of
          Right (_, state) -> currentPosition state `shouldBe` Position 2 1 1
          Left _ -> expectationFailure "Lexer failed"
  ...
  describe "Error position tracking" $ do
    it "reports correct position on error" $ do
      ...
      case result of
        Left err -> (err ^. errorPosition) `shouldBe` Position 1 1 0
    ...
    it "reports correct position on error mid-input" $ do
      ...
      case result of
        Left err -> (err ^. errorPosition) `shouldBe` Position 1 3 2
        Right _ -> expectationFailure "Should have failed"
    ...
    it "reports correct position on error after newline" $ do
      ...
      case result of
        Left err -> (err ^. errorPosition) `shouldBe` (Position 2 1 1)
        Right _ -> expectationFailure "Should have failed"
